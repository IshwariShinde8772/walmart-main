...
def send_security_alert_email(to_email, username, reason):
...
def login():
    ...
    if request.method == 'POST':
        ...
        hour = now.hour
        trust_score, is_suspicious, new_location = ml_predict_trust_score(hour, geo_distance, failed_attempts, api_rate)
        
        print(f"DEBUG LOGIN: User: {username}, Trust Score: {trust_score}, Is Suspicious: {is_suspicious}")
        ...
        # Trust score logic:
        # Admin users bypass all security checks for direct access
        if user['is_admin']:
            print(f"DEBUG LOGIN: Admin user {username}, bypassing security checks")
            session['user_id'] = user['id']
            session['is_admin'] = user['is_admin']
            session['username'] = user['username']
            return redirect(url_for('admin_dashboard'))
        ...
        if trust_score < 50:
            print(f"DEBUG LOGIN: Low trust score ({trust_score}), requiring email verification")
            ...
            print(f"DEBUG LOGIN: Trust score ({trust_score}), direct login")
            # Trust score >= 50 - direct login
            if is_suspicious:
                send_security_alert_email(user['email'], user['username'], 'Suspicious login attempt detected')
...
def employee_dashboard():
    print(f"DEBUG EMPLOYEE: Session user_id: {session.get('user_id')}, is_admin: {session.get('is_admin')}")
    if not session.get('user_id') or session.get('is_admin'):
        print("DEBUG EMPLOYEE: Redirecting to login - no user_id or is_admin")
        return redirect(url_for('login'))
    ...
    # Fetch login attempts for this user
    ...
    # Get last login time
    last_login_time = "Never" if not logs else logs[0].get('timestamp', 'Unknown')
    ...
    for log in logs:
        if log.get('timestamp'):
            try:
                ...
                log['timestamp'] = utc_time.strftime('%Y-%m-%d %H:%M:%S')
            except Exception as e:
                log['timestamp'] = 'Unknown'
    
    # Set default values for missing fields
    user.setdefault('trust_score', 0)
    user.setdefault('email', 'No email')
    user.setdefault('username', session.get('username', 'Unknown'))
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user_exists:
                ...
                trust_score, is_suspicious, new_location = ml_predict_trust_score(hour, geo_distance_km, failed_attempts, api_rate)
                ...
                return render_template('create_account.html', error=error, success=success, redirect=True, delay=3)
                
        except Exception as e:
            print(f"Supabase error during account creation: {e}")
